    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative min-h-[320px] flex flex-col items-center justify-center pt-32 pb-24 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          <div className="absolute top-0 left-0 w-96 h-96 bg-primary/10 rounded-full filter blur-3xl opacity-60" />
          <div className="absolute bottom-0 right-0 w-96 h-96 bg-primary/10 rounded-full filter blur-3xl opacity-60" />
        </div>
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid gap-8">
          {statuses.length === 0 ? (
            <div className="bg-card rounded-2xl shadow-lg border border-border p-8 text-center hover:shadow-xl transition-all duration-300">
              ...
              <p className="text-muted-foreground text-lg">There are no active issues at this time.</p>
            </div>
          ) : (
            statuses.map((status) => (
              <div key={`status-${status.ticketId || 'unknown'}`} className="bg-card rounded-2xl shadow-lg border border-border overflow-hidden hover:shadow-xl transition-all duration-300">
                {/* Status Header */}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <h2 className="text-2xl font-bold text-foreground">{status.title || 'System Issue'}</h2>
                      </div>
                    ...
                    <div className="text-right ml-6">
                      <span className="text-sm font-semibold text-accent-foreground bg-accent px-3 py-1 rounded-full">
                        Incident #{status.ticketId || 'Unknown'}
                      </span>
                      {status.application && (
                        <div className="mt-3">
                          <div className="inline-flex items-center gap-2 px-3 py-2 bg-accent rounded-xl">
                            ...
                            <span className="text-sm font-semibold text-accent-foreground">{status.application}</span>
                          </div>
                        </div>
                      )}
                ...
                <div className="p-8">
                  <div className="prose max-w-none">
                    <p className="text-foreground text-lg leading-relaxed mb-6">{status.message || 'No message available'}</p>

                    {status.affectedServices && status.affectedServices.length > 0 && (
                      <div className="mb-6">
                        <h3 className="text-lg font-semibold text-foreground mb-3">Affected Services</h3>
                        <div className="flex flex-wrap gap-3">
                          {status.affectedServices.map((service, index) => (
                            <span
                              key={service || index}
                              className="px-3 py-1 rounded-full text-sm font-medium bg-muted text-foreground border border-border"
                            >
                    ...
                    {status.updates && (
                      <div>
                        <h3 className="text-lg font-semibold text-foreground mb-3">Updates</h3>
                        <div className="text-foreground text-base leading-relaxed whitespace-pre-line bg-muted p-4 rounded-xl border border-border">{status.updates}</div>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {status.publicStatus === 'Resolved' && (
                  <div className="bg-success/10 px-8 py-6 border-t border-success/20">
                    <div className="flex items-center gap-3">
        ...
        <div className="mt-16 text-center">
          <div className="bg-muted rounded-2xl p-6 border border-border">
            <p className="text-muted-foreground mb-2">Page last updated: {formatDate(new Date(lastPageUpdate).toISOString())}</p>
            {statuses.some(status => status.publicStatus !== 'Resolved') && (
              <p className="text-muted-foreground">
                Next update in <span className="font-semibold text-primary">{formatCountdown(timeUntilRefresh)}</span>
              </p>
            )}
          </div>
        </div>
